# Курсовой проект по курсу "DevOps-инженер с нуля: расширенный курс" Ячмень Марк Викторович
...
Общая схема развернутой инфраструктуры:
...
Для централизованного логирования используется стек:

- **Filebeat** — сбор логов nginx
- **Elasticsearch** — хранение логов
- **Kibana** — просмотр логов
...
Просмотр логов nginx через Kibana:

![Kibana Logs](img/kibana_logs.png)

---

# Балансировка нагрузки

В инфраструктуре используется Application Load Balancer (ALB),
который распределяет HTTP-трафик между веб-серверами.

При обновлении страницы происходит переключение между:
...
![Web via ALB](img/web_via_alb.png)
...
Студент курса "DevOps-инженер с нуля: расширенный курс" Ячмень Марк Викторович
 
Образовательная платформа: Netology
