...
До создания Service Pod `nginx-init` находится в состоянии `Init:0/1`: init-контейнер ожидает появления `nginx-init-service`, а основной контейнер `nginx` ещё не запущен.
...
После создания Service `nginx-init-service` init-контейнер успешно завершил работу, основной контейнер `nginx` был запущен, а Pod перешёл из состояния `Init:0/1` в состояние `1/1 Running`.
