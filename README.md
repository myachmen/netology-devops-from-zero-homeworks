# Домашнее задание по теме "Система сбора логов Elastic Stack" Ячмень Марк Викторович
...
В выводе присутствуют 6 контейнеров: пять компонентов Elastic Stack
(`es-hot`, `es-warm`, `kibana`, `logstash`, `filebeat`) и дополнительный
контейнер `some_app` из предоставленного в задании `docker-compose.yml`.
Контейнер `some_app` является dummy-приложением и используется для
генерации тестовых логов.
